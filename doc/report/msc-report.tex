...
In this thesis we present a data store called Zohmg. Zohmg is built on top of
HBase and uses Hadoop to run import jobs in parallel. It solves the problem of
summing, storing and serving measurements. These measurements may be taken
from log files or similar sources. Zohmg stores only the summarized data, not
the atomic measurements. The data store is optimized for speed of data
retrieval: one of the design goals was to serve data at mouse-click rate and
promote real-time data exploration.
...
analysis. As the services grow in size, so will the data sets they generate grow.

An important property of these data sets is that they generally have a regular
structure and are represented in independent records. The records can be
broken up without losing any of their meaning and are as such very well suited
for parallel analysis. The data sets are not fit to store using traditional
database techniques, if only for the fact that they are too large to fit in a
relational database.
