...
is most popular in a certain country, etc.
...
Time series is one of the most important measures in log analyzing, it
is also very natural when the data is time-stamped. \cite{discoveringweb}


\section{Aggregates}

% TODO: be clear about how aggregates are *derived* from one or several vars.
% TODO: example.
...
This chapter will discuss and analyze the system that is to be built. First the
function wanted from the system is presented; while discussing the input and
output characteristics, and possible challenges. Secondly, a rough sketch and
discussion of the system's different parts will be presented. Thirdly, the
requirements of the system will be featured.

Throughout this chapter distinction is made between the developer: who develops
the system, and the user: who uses the product which the developer
...
presented to the user as graphs. Drawbacks of the system include problems with
new data source additions: they can sometime not be loaded because they are to
big. If a user wants to perform a custom query, manual intervention by the
developer is necessary. There is no way for the system to handle custom queries.
...
Reporting the analyzed is the data is the final step. This is what the user is
...
exist for custom queries. Enabling the user to query the aggregated data---for
...
The following three components form the outline of the system: importing,
...
listen to, this is called scrobbling.}, radio etc. This data reside on a
commodity hardware cluster used for parallel computation. The system needs to be
able to easily import data from this source.
...
of applications. Therefor the system does not implement any user interface, this
is left for the developer to implement.


\section{System requirements}

The four key requirements of the system are: ease of use, speed, scalability,
and being able to fit in the existing software and hardware eco-system.
...
Another technique is parallelizing, using a cluster of commodity hardware
...
% scaling.
In order for a computational setup, cluster or otherwise, to be efficient it
needs to scale well, this means adding more hardware should add as close as
possible to the hardware's potential addition to the computational power.
...
The system function part discussed the system's goals and parts. The goals are
to have a system which enables the developer to easily import and analyze new
data sources and to be fast enough to serve the analyzed data at mouse-click
rate to the user. The system's three core parts---importing, storing, and
extracting data---have also been discussed.

Furthermore the requirements on the system were discussed; the four key
