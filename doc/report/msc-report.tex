...
Secondly, this chapter features new techniques in distributed storage and
computing; these techniques are Google's BigTable and MapReduce.
...
MongoDB, Hypertable. CouchDB is an document-oriented database.
...
An example data source is the Apache HTTP Server access logs. The Apache HTTP
Server Combined Log Format is commonly used to log requests. The log format is
...
Combined Log Format.,captionpos=b]
   ...
   "GET /music HTTP/1.0" 200 2326 "http://last.fm/"
...
the entry would be on one line. The example log line contains access info for a
specific request; it contains the key entries: the client's IP address,
timestamp, requested resource, HTTP status code for the request, the client's
user agent etc.

Revisiting the running example of Apache access logs, a usermap function might
look something like this:

\vspace{12pt}

\begin{lstlisting}[language=Python,caption=An example map function.,captionpos=b]
   def map(key, value):
...
useragents that are not web spiders which resulted in a HTTP 200 status code.
...
\cite{definitive_guide_to_pylons,paste} Since the main goal was not to build a
...
depending on the URL it was fed. \cite{python_paste}
...
The transformers application fetches data from HBase with the same mechanism as
the raw data application. The data is transformed with a transformer---a Python
program in the project's \texttt{transformers} directory---requested by the
user. Similar to the raw data serving, the output is served as JSONP.
