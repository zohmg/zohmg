...
Generally, the services and operations will generate a log for later analysis.
As the services grow in size, so will the data sets they generate.
...
each country every month (see table 1.1).
...
% TODO: renskriv these thoughts.

Last.fm is an internet radio station focusing on content, being a musical
encyclopedia. Last.fm lets users submit what music they listen to, called
\textit{scrobbling}, this user submitted data is analyzed and Last.fm offer
their users personalized charts, music and event recommendations, based ther
listening. Last.fm also streams content, via internet, to users. The Last.fm web
site ties together all these areas and presents it to the users.
...
In chapter 2 general data warehous concepts are introduced; it also introduces
...
schema would be used to modell the dimensions.
...
row is indexed with a row key, a column key, and a timestamp. The columns have
...
The first section in this chapter will present the work procedure during the
master thesis. The following chapters will discuss selected tools and their
impact on design and implementation choices for the Zohmg system.


\section{Procedure}

% TODO: generally use passive tense.
% TODO: renskriv.
...
good understanding of the problem at hand, and how we successively moved closer
...
are described, followed by a summary of our tools studies.


\subsection*{Beginning}

% not so sure I like this section much..
...
The possible tools were explored by testing and benchmarking them.

The problem area was explored by prototyping solutions to a given subproblem.


\subsubsection*{Software}

% TODO: renskriv.
MapReduce frameworks: Hadoop, disco.

Hadoop is used in production at Last.fm, it made sense to use it because the
work done would integrate with the Last.fm's infrastructure easily.
...
These databases were explored and tried out. Sadly most of these BigTable-like
databases were in a very early phase of development and hard to even install.
The experience was that HBase was the most mature of them. Maturity was the
compound experience with the software: community, codebase, documentation,
stability etc. HBase is also tightly coupled with Hadoop which is used in
production at Last.fm.

CouchDB was experienced to be mature as well but lacked integration with
MapReduce, all database communication would be necessary to do over HTTP.
Setting up CouchDB with replication required some hands-on, it lacks sorting of
documents which BigTable-like databases utilize.

\subsubsection*{Benchmarks}

Only HBase made it this far, the other databases in the roster were hard or
impossible to either compile, install, setup, or all of them.

HBase was benchmarked (the benchmarks was similar to the ones found in the
BigTable paper?).


\subsubsection*{Prototyping}

The problem domain was explored by asking, the system to be built and the
...
% this is a little thin. The prototypes are small pieces of code that would
TODO: answer questions such as Is it at all possible to.. ? and What is the most
efficient way to..?


\subsection*{Literature studies}
% vet inte riktigt vad jag vill saga om det har.
...
as the design phase that led up to the implementation.

The following sections presented and discussed the selected tools; they also
discussed the impact on design and implementation choices.
...
implementation details of the Zohmg system's exporting compontent.


\section{Project Management}

A Zohmg project is necessary for using Zohmg; they are created with a management
...
A standard MapReduce map function emits key-value pairs. The Zohmg usermap
function, instead, emits triples (tuples with three values), which consist of a
timestamp, a hash table of dimensions and their respective values, and a hash
table with measurements (values). In effect, the dimensions and the timestamp
...
address, timestamp, requested resource, HTTP status code for the request, the
...
in n-space (the dimensions and the timestamp) is emitted together with a count
of pageviews and bytes; one pageview per request and the number of bytes served
in the request. It is possible to add logic here, perhaps only counting requests
from useragents that are not web spiders which resulted in a HTTP 200 status
code.
...
The above query would return the pageviews for SE between 1 January
...
a Python program in the project's \texttt{transformers} directory. See the
...
system. The concept of Zohmg project's and management of thesame were
